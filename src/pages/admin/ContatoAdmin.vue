<template>
  <div v-if="currentUser" class="row justify-center relative-position">
    <div style="width:80vw; max-width: 1200px">
      <div>
        <h1 class="primaryGradiente text-center text-uppercase text-weight-bolder" style="width:80vw; max-width: 400px">Contato</h1>
      </div>
      <div class="row items-start justify-around">
        <div class="col-md-10 col-xs-10 q-gutter-md">
          ...
          <div>
            <q-input outlined v-model="form.data.whatsapp" type="text" mask="(##)#####-####" label="Whatsapp" />
          </div>
          <div class="row items-center justify-around">
            <BtnAdmin :form="form" collection="contato" @onReset="onReset" :validarDados="validarDados" />
          </div>
        </div>
      </div>
    </div>
...
let contato = computed(() => global.contato)
...
let validarDados = computed(() => {
  ...
  if(isForm?.whatsapp && isForm?.text ) {
    return true
  } else {
    return false
  }
...
async function onSubmit() {
  form.value.data.whatsapp = await formataTel(form.value.data.whatsapp)
...
async function formataTel(item) {
  return item.replace(/[^0-9]/g,'')
}

onMounted(() => {
  if(contato.value) {
    setForm(contato.value)
  }
})
