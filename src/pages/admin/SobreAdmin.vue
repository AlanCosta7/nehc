<template>
  <div v-if="currentUser" class="row justify-center relative-position">
    <div style="width:80vw; max-width: 1200px">
      <div>
        <h1 class="primaryGradiente text-center text-uppercase text-weight-bolder" style="width:80vw; max-width: 400px">Sobre</h1>
      </div>
      <div class="row items-start justify-around">
        <div class="col-md-10 col-xs-10 q-gutter-md">
          ...
          <div>
            <q-input outlined v-model="form.data.missao" type="textarea" label="Missão" />
          </div>
          <div>
            <q-input outlined v-model="form.data.visao" type="textarea" label="Visão" />
          </div>
          <div>
            <q-input outlined v-model="form.data.valores" type="textarea" label="Valores" />
          </div>
          <div class="row items-center justify-around">
            <BtnAdmin :form="form" collection="sobre" @onReset="onReset" :validarDados="validarDados" />
          </div>
        </div>
      </div>
    </div>
...
let sobre = computed(() => global.sobre)
...
let validarDados = computed(() => {
  ...
  if(isForm?.missao && isForm?.visao && isForm?.text && isForm?.valores ) {
    return true
  } else {
    return false
  }
