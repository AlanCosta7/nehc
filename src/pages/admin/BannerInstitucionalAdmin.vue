      ...
      <div class="row items-start justify-around">
        <div class="col-md-10 col-xs-10 q-gutter-md">
          <div class="row items-start q-gutter-md">
            <div>
              <h1 class="primaryGradiente text-uppercase text-weight-bolder" style="width:80vw; max-width: 800px">Quem somos</h1>
            </div>
            <div class="col-12">
              <q-img width="300px" v-if="form.id" height="300px" :src="form.data.imagem_600" @click="onPickPhoto" class="cursor-pointer relative-position">
                <template v-slot:error>
                  ...
                  <div>
                    <q-btn style="width: 300px" color="negative" icon="close" outline label="remover imagem" @click="onExcluir()" />
                  </div>
            </div>
          ...
          <div class="row items-center justify-around">
              <BtnAdmin :form="form" collection="banner-institucional" :validarDados="validarDados" />
          </div>
        </div>
      </div>
    </div>
...
let bannerInstitucional = computed(() => global.bannerInstitucional[0])
...
let validarDados = computed(() => {
  ...
  if(isForm?.title && isForm?.subtitle ) {
    return true
  } else {
    return false
  }
...
async function onSubmit() {
  if(form.value.id) {
    let val = {
      collection: 'banner-institucional',
...
onMounted(() => {
  form.value = bannerInstitucional.value || {
    id: null,
    data: {
      title: null,
      subtitle: null,
